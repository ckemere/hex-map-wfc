...
function buildDecorationMap(grid) {
  ...
  addItems(decs.waterlilies, mesh, (l, pos) => {
    ...
    const def = TILE_LIST[l.tile.type]
    const dip = (name.startsWith('COAST_') || def?.waterSubtype) ? -0.2 : 0
    return {
  ...
  addItems(decs.rocks, mesh, (r, pos) => {
    ...
    const rDef = TILE_LIST[r.tile.type]
    const dip = rDef?.waterSubtype ? -0.2 : (name.startsWith('COAST_') || name.startsWith('RIVER_')) ? -0.1 : 0
    return {
